...
ppt_files <- list.files('.', pattern='^ppt.*tif$')[1:468]
...
pet_files <- list.files('.', pattern='^pet.*tif$')[1:468]
...
tmp_files <- list.files('.', pattern='^tmp.*tif$')[1:468]
...
foreach(n=1:nrow(rll), .packages=c('raster', 'gdalUtils', 'SPEI', 'dplyr', 'zoo')) %dopar% {
  ...
  tmp <- extract(tmp_vrt_file, rll$x[n], rll$y[n]) - 273.15
