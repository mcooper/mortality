...
data <- data %>%
  select(mortality, age, mother_years_ed, mothers_age, birth_order, male, x, y, 
				 ...
				 ct.tmax.30, ct.tmax.35, ct.tmax.40) %>%
  na.omit
...
###ct.tmax.30
mod <- bam(mortality ~ age + mother_years_ed + mothers_age + birth_order + male + s(y, x, bs='sos') + s(ct.tmax.30),
           family='binomial', data=data, cluster=cl)
...
###ct.tmax.35
mod <- bam(mortality ~ age + mother_years_ed + mothers_age + birth_order + male + s(y, x, bs='sos') + s(ct.tmax.35),
           family='binomial', data=data, cluster=cl)
...
###ct.tmax.40
mod <- bam(mortality ~ age + mother_years_ed + mothers_age + birth_order + male + s(y, x, bs='sos') + s(ct.tmax.40),
           family='binomial', data=data, cluster=cl)
