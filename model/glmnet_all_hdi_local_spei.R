...
for (i in c(1, 2, 3, 6, 12, 24, 36, 48)){
  ...
  #ERA5
  form <- paste0("mortality ~ age + mother_years_ed + birth_order + male + shdi*spei.er.", i, " + shdi*I(pmax(spei.er.", i, " + 1, 0)) + shdi*I(pmax(spei.er.", i, " - 0, 0)) + shdi*I(pmax(spei.er.", i, " - 1, 0)) + healthindex + incindex + edindex")
  mm <- sparse.model.matrix(as.formula(form), data=data)

  mod <- glmnet(mm, data$mortality, family='binomial', alpha=1, trace.it=1)
  save(mod, file=paste0('~/mortalityblob/glmnet/spei.er.', i, '.all.hdi.local.Rdata'))
  rm(mm)

  #FLDAS
  form <- paste0("mortality ~ age + mother_years_ed + birth_order + male + shdi*spei.fl.", i, " + shdi*I(pmax(spei.fl.", i, " + 1, 0)) + shdi*I(pmax(spei.fl.", i, " - 0, 0)) + shdi*I(pmax(spei.fl.", i, " - 1, 0)) + healthindex + incindex + edindex")
  mm <- sparse.model.matrix(as.formula(form), data=data)

  mod <- glmnet(mm, data$mortality, family='binomial', alpha=1, trace.it=1)
  save(mod, file=paste0('~/mortalityblob/glmnet/spei.fl.', i, '.all.hdi.local.Rdata'))
  rm(mm)

  #TerraClimate
  form <- paste0("mortality ~ age + mother_years_ed + birth_order + male + shdi*spei.tc.", i, " + shdi*I(pmax(spei.tc.", i, " + 1, 0)) + shdi*I(pmax(spei.tc.", i, " - 0, 0)) + shdi*I(pmax(spei.tc.", i, " - 1, 0)) + healthindex + incindex + edindex")
  mm <- sparse.model.matrix(as.formula(form), data=data)

  mod <- glmnet(mm, data$mortality, family='binomial', alpha=1, trace.it=1)
  save(mod, file=paste0('~/mortalityblob/glmnet/spei.tc.', i, '.all.hdi.local.Rdata'))
  rm(mm)
}
