...
for (i in c(1, 2, 3, 6, 12, 24, 36, 48)){
  ...
  mod <- glmnet(mm, data$mortality, family='binomial', alpha=0, trace.it=1)
  save(mod, file=paste0('~/mortalityblob/glmnet/spei.er.', i, '.all.hdi.ridge.Rdata'))
  rm(mm)
  ...
  mod <- glmnet(mm, data$mortality, family='binomial', alpha=0, trace.it=1)
  save(mod, file=paste0('~/mortalityblob/glmnet/spei.fl.', i, '.all.hdi.ridge.Rdata'))
  rm(mm)
  ...
  mod <- glmnet(mm, data$mortality, family='binomial', alpha=0, trace.it=1)
  save(mod, file=paste0('~/mortalityblob/glmnet/spei.tc.', i, '.all.hdi.ridge.Rdata'))
  rm(mm)
}
